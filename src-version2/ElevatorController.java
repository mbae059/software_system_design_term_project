...
class ElevatorController {
	...
	public void doorTimeout() {
		...
		if (shouldMove()) {
			moveElevator(nextDirection);
		}
	}

	public boolean shouldMove() {
		return getNextDirection() != Direction.IDLE;
	}
	private Direction getNextDirection() {
		if(floorstobeVisited.isEmpty()) return Direction.IDLE;

		Floor destination = floorstobeVisited.getFirst();

		if(destination.isUpperThan(destination)) return Direction.UP;
		else return Direction.DOWN;
	}

	public void assertDoors() {
		assert elevatorDoor != null;
