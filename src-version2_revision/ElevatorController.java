...
class ElevatorController {
	...
	private ElevatorStopStrategy stopStrategy; // Using the Strategy Interface(step2)
...
//							  JavaDoorTimer doorTimer) 
	...
	public ElevatorController(DeviceVendor deviceVendor, ElevatorMotor elevatorMotor,
            List<FloorDoor> floorDoors, JavaDoorTimer doorTimer){//(step2)
		
		//Configuring the Strategy Interface(step2)
		if (elevatorControllerKind == ElevatorControllerKind.EVERY_FLOOR_STOP) {
            this.stopStrategy = new EveryFloorStopStrategy();
        } else {
            this.stopStrategy = new DestinationFloorStopStrategy();
        }
		
//		this.elevatorControllerKind = elevatorControllerKind;
		this.elevatorMotor = elevatorMotor;
//		this.elevatorDoor = elevatorDoor;
		this.elevatorDoor = ElevatorDoorFactory.createElevatorDoor(deviceVendor);//£¨step2£©
...
//			needToStop = getFloorsToBeVisited().contains(floor);
		boolean needToStop = stopStrategy.shouldStop(this, floor);//(step2)
...
//		final int destination = floorsToBeVisited.getFirst() ;
		final int destination = floorsToBeVisited.get(0);
